...
// ToQuery returns the replayable form of the call: a SQL string with ordered
// @p1..@pN bind markers and the positional args, type-coerced when a
// declaration provides enough info. When decl is missing, values are passed as
// strings and the driver does its best.
//
// SQL Server accepts only @pN bind markers, so the markers are written here
// rather than left as `?` for a driver to translate.
func (c *RPCCall) ToQuery() (string, []any, []coerceWarning) {
	...
	// Rewrite the call's own @PN names to ordered @pN bind markers. The
	// replacement runs in declaration order so positional args line up with
	// the first textual appearance of each name. \b keeps @P1 from matching
	// inside @P10, and the markers are lower-case so a name rewritten early
	// cannot be matched again by a later, upper-case pattern.
	sqlText := c.Template
	for index, p := range placeholders {
		re := regexp.MustCompile(regexp.QuoteMeta(p.Name) + `\b`)
		sqlText = re.ReplaceAllLiteralString(sqlText, fmt.Sprintf("@p%d", index+1))
	}
...
func isReplayable(e Event, template string) (bool, string) {
	...
	// A statement captured from inside a stored procedure refers to that
	// procedure's parameters and local variables. Neither is in the trace, so
	// re-running it standalone can only fail with "must declare the scalar
	// variable" — skip it with a reason rather than emit a guaranteed error.
	if e.Name == EventSPStatementCompleted {
		return false, "statement inside a stored procedure"
	}
	if e.ParamsUnavailable {
		return false, "parameters not captured"
	}
	if template == "" {
...
func ReplayOne(ctx context.Context, db *sql.DB, e Event) ReplayResult {
	...
	if call, ok := ParseRPC(e.Statement); ok {
		...
		sqlText, args, warnings = call.ToQuery()
	} else {
