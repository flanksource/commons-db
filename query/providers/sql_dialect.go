...
func validateSQLIdentifierPath(identifier string) ([]string, error) {
	parts := strings.Split(identifier, ".")
	for index, part := range parts {
		name, ok := allowlist.Copy(part, sqlIdentifierAlphabet)
		if !ok || !validSQLIdentifier.MatchString(name) {
			return nil, fmt.Errorf(
				"%q is not a valid SQL identifier; use plain names separated by dots", identifier)
		}
		parts[index] = name
	}
	return parts, nil
...
func (d sqlDialect) quoteIdentifierPath(identifier string) (string, error) {
	parts, err := validateSQLIdentifierPath(identifier)
	if err != nil {
		return "", err
	}
	for index, part := range parts {
		parts[index] = d.quoteValidatedIdentifier(part)
	}
	return strings.Join(parts, "."), nil
}

func (d sqlDialect) quoteValidatedIdentifier(name string) string {
	switch d {
	case dialectMySQL:
		return "`" + name + "`"
	case dialectSQLServer:
