...
var _ = Describe("sql column filters (postgres)", Ordered, func() {
	...
	It("binds a scalar profile parameter instead of injecting it into SQL", func() {
		profile := profileFor("SELECT id, region, env FROM orders WHERE region = {{.params.region}} ORDER BY id")
		profile.Params = []query.ParamDef{{Name: "region", Type: query.ParamTypeString}}

		result, err := query.Execute(context.New(), profile, map[string]any{
			"region": "us-east' OR TRUE --", "filter.env": "prod",
		})
	...
	It("expands a list profile parameter into separately bound values", func() {
		profile := profileFor("SELECT id, region FROM orders WHERE region IN ({{.params.regions}}) ORDER BY id")
		profile.Params = []query.ParamDef{{Name: "regions", Type: query.ParamTypeList}}

		result, err := query.Execute(context.New(), profile, map[string]any{
			"regions": []string{"us-east", "eu') OR TRUE --"},
		})

		Expect(err).ToNot(HaveOccurred())
		Expect(ids(result.Rows)).To(Equal([]int{1, 4}))
	})

	It("quotes database, table, and column parameters while binding values", func() {
		profile := profileFor(
			"SELECT id, region, env FROM {{.params.table}} " +
				"WHERE {{.params.column}} = {{.params.value}} ORDER BY id",
		)
		profile.Params = []query.ParamDef{
			{Name: "table", Type: query.ParamTypeIdentifier},
			{Name: "column", Type: query.ParamTypeIdentifier},
			{Name: "value", Type: query.ParamTypeString},
		}

		result, err := query.Execute(context.New(), profile, map[string]any{
			"table": "public.orders", "column": "env", "value": "prod",
		})

		Expect(err).ToNot(HaveOccurred())
		Expect(ids(result.Rows)).To(Equal([]int{1, 2, 5}))
	})
	...
	Describe("value lookup", func() {
		...
		It("materializes profile parameters before adding lookup predicates", func() {
			profile := profileFor("SELECT id, region, env FROM {{.params.table}} WHERE env = {{.params.env}} ORDER BY id")
			profile.Params = []query.ParamDef{
				{Name: "table", Type: query.ParamTypeIdentifier},
				{Name: "env", Type: query.ParamTypeString},
			}

			options, _, err := query.LookupFilterValues(context.New(), query.FilterValueLookupRequest{
				Profile: profile,
				Input:   map[string]any{"table": "public.orders", "env": "prod"},
				Key:     "filter.region",
				Limit:   20,
			})
	...
	It("materializes profile parameters during column inspection", func() {
		profile := profileFor("SELECT id, region, env FROM {{.params.table}} WHERE env = {{.params.env}} ORDER BY id")
		profile.Params = []query.ParamDef{
			{Name: "table", Type: query.ParamTypeIdentifier},
			{Name: "env", Type: query.ParamTypeString},
		...
		result, err := query.Sample(context.New(), profile, query.SampleOptions{
			Params:     map[string]any{"table": "public.orders", "env": "prod"},
			Inspection: query.InspectionOptions{Refresh: true},
		})

		Expect(err).ToNot(HaveOccurred())
		Expect(result.Inspection).ToNot(BeNil())
		Expect(result.Inspection.Status).To(Equal("complete"))
	})
