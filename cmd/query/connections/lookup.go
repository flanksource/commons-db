...
// types valid for the selected provider via the `types` flag (ListOptions.Types).
type connectionFilter struct{ service *Service }
...
func (connectionFilter) Lookup(*ListOptions) (map[string]api.Textable, error) { return nil, nil }

// Options enumerates every in-scope connection (the non-search path).
func (f connectionFilter) Options(opts ListOptions) map[string]api.Textable {
...
func (f connectionFilter) OptionsWithQuery(opts ListOptions, query string, limit int) (map[string]api.Textable, int) {
	db, err := f.service.database()
	if err != nil {
		return nil, 0
	}
