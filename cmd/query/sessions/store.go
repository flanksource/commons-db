...
// Store is the durable record of trace/top sessions: its OnTransition
...
func (s *Store) OnTransition(info query.SessionInfo) {
	...
	err := s.db.WithContext(context.Background()).Clauses(clause.OnConflict{
...
func (s *Store) List(ctx context.Context) ([]query.SessionInfo, error) {
	...
	if err := s.db.WithContext(ctx).Order("started_at DESC").Find(&records).Error; err != nil {
...
// Prune atomically deletes terminal sessions older than the retention window
// and their events.
